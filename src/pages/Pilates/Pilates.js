...
import CarouselPilates from './components/CarouselPilates';
import Slider from 'react-slick';
import { getPilates, url_image } from '../../service/api';
...
export default function Pilates() {
  ...
  const getData = async(e) => {
    const res = await getPilates();
    setPilatesData(res.data?.data);
    // console.log(res.data.data, "res get Pilates");
  }
  ...
  const settings_training = {
    ...
    nextArrow: <SampleNextArrow disabled={sliderIndex === pilatesData?.items_row3?.length - 1} />,
  };

  return (
    <Box className='bg-content home d-flex flex-column fuel'>
      <Header />
      <Box sx={{ backgroundImage: `url(${url_image(pilatesData.r1_bg)})` }} className='d-flex justify-content-center align-items-center header-image'>
        <Fade right cascade>
          <Box><p className='s-96 Oswald-700'>{pilatesData.r1_title}</p></Box>
        </Fade>
      </Box>
      <div className='container-content content'>
        <Fade delay={150}>
          <Box className='text-center pb-text-content'>
            <p className='s-4050 Din-700'>{pilatesData.r2_text_top}</p>
          </Box>
        </Fade>
        <Fade delay={250}>
          <Box className='text-center'
            sx={{ '@media (min-width: 768px)': { px: 10 } }}>
            <p className='Din-400 s-2030'>{pilatesData.r2_text_bottom}</p>
          </Box>
        </Fade>
      </div>
      <Box>
        <Box className='carousel-fc-header-position' sx={{ pb: 5 }}>
          <Box className={`d-flex align-items-center ${smallDeviceSize ? 'justify-content-center' : ''}`}>
            <p className='Oswald-700 s-6495'>{pilatesData.r3_title}</p>
          </Box>
          {!smallDeviceSize && pilatesData?.items_row3?.length > 2 && <Box className='d-flex justify-content-center align-items-center'>
      ...
      <Box className='carousel-fc-spacing'>
        {smallDeviceSize ?
          <Box className='d-flex flex-md-row flex-sm-column col-12 justify-content-center'>
            ...
            {pilatesData?.items_row3?.map((item) => (
                <Box className='d-flex justify-content-center'>
                  <Box key={item.title} className='carousel-fc-img-sz d-flex flex-column justify-content-end pointer trans' sx={{ backgroundColor: item.image ? '' : '#D9D9D9', backgroundImage: `url(${url_image(item.image)})` }}>
                    ...
                    {/* <ul className='carousel-fc-details mb-4' style={{ opacity: '0', transition: '0.4s all', transform: 'translateY(20px)' }}>
                      {item?.text.map((d, i) => {
                        return (
                          <li><Typography fontSize={smallDeviceSize ? '16px' : '28px'} className='Din-400'>{d}</Typography></li>
                        )
                      })}
                    </ul> */}
