...
function CarouselItemsComp(props) {
    ...
    <Box className='d-flex flex-column justify-content-end align-items-start fc-carousel'
      ...
      <Box position='relative' sx={{ pl: '1rem' }} className="trans fc-box">
        ...
        <Box className='carousel-fc-details  ' sx={{ opacity: '0', transition: '0.4s all' }}>
          ...
          <p className='Din-400 s-2426 fc-hide trans' dangerouslySetInnerHTML={{__html: item.text}}></p>
        </Box>
...
export default function CarouselFunctional(props) {
  const { functionalData, carouselItems, reactScrollComponentRef, carouselComponentRef, carouselItemsRef, handleNext, handleBack } = props;
        ...
        <div ref={carouselItemsRef} className='d-flex align-items-center'>
          <div className='d-flex flex-row'>
            {functionalData?.items_row3?.map((item) => {
              return (
                <CarouselItemsComp item={item} smallDeviceSize={smallDeviceSize} largeDeviceSize={largeDeviceSize} />
              )
            })}
          </div>
        </div>
